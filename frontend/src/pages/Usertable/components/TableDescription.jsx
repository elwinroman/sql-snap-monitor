...
export function TableDescription() {
  const userTableExtendedPropertieList = useUserTableStore((state) => state.userTableExtendedPropertieList)
  const userTableColumnList = useUserTableStore((state) => state.userTableColumnList)
  const userTableIndexList = useUserTableStore((state) => state.userTableIndexList)
  const userTableForeignKeyList = useUserTableStore((state) => state.userTableForeignKeyList)

  // Busca la columna que es clave primaria (solo puede haber una por ahora)
  const primaryKeyObject = userTableIndexList.find((element) => element.isPrimaryKey === true)

  // Formatear las claves foráneas agregando el nombre de la columna como clave
  const foreignKeysFormattedList = userTableForeignKeyList.reduce((acc, item) => {
    ...
    <div>
      <div className="py-4">
        ...
        {userTableExtendedPropertieList.length === 0 && (
          <div key={uuidv4()} className="flex gap-5 rounded-sm border border-dashed border-zinc-500 px-4 py-2">
            <p className="flex items-center gap-4 text-sm">
              <span className="text-zinc-500">No existe descripción de tabla</span>
              <span className="text-zinc-500">No existe nombre de propiedad</span>
            </p>
          </div>
        )}
      </div>
      <Table>
        ...
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-nowrap font-semibold">Nombre de propiedad</TableHead>
          </TableRow>
        </TableHeader>
        ...
        {userTableColumnList.length > 0 && (
          <TableBody>
            {userTableColumnList.map((item, index) => (
              <TableRow key={uuidv4()}>
                <TableCell className="w-[20px] text-zinc-200">{index + 1}</TableCell>
                    ...
                    {primaryKeyObject && primaryKeyObject.columnId === item.id && (
                      <TooltipProvider>
                        <Tooltip delayDuration={100}>
                          ...
                          <TooltipContent>
                            <span className="text-amber-400">{primaryKeyObject.isPrimaryKey ? 'clave primaria' : ''}</span>
                            <span>{`${primaryKeyObject.isUnique ? ', unique, ' : ''}${primaryKeyObject.typeDesc.toLowerCase()}`}</span>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    )}
                ...
                <TableCell>
                  <div className="flex flex-col gap-2 text-zinc-200">
                    ...
                    {item.extendedProperties.length === 0 && <span className="text-zinc-500">No existe descripción</span>}
                  </div>
                </TableCell>
